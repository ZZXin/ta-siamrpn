...
class DepthwiseXCorr(nn.Module):
    ...
    def forward(self, kernel, search, frame_id=None, layer_id=None, branch=None):
...
class DepthwiseRPN(RPN):
    ...
    def forward(self, z_f, x_f,frame_id=None, layer_id=None):
        cls = self.cls(z_f, x_f, frame_id=frame_id, layer_id=layer_id, branch="cls")
        loc = self.loc(z_f, x_f, frame_id=frame_id, layer_id=layer_id, branch="loc")
        return cls, loc
...
class MultiRPN(RPN):
    ...
    def forward(self, z_fs, x_fs,frame_id = None):
        cls = []
        loc = []
        for idx, (z_f, x_f) in enumerate(zip(z_fs, x_fs), start=2):
            rpn = getattr(self, 'rpn'+str(idx))
            c, l = rpn(z_f, x_f, frame_id=frame_id, layer_id=idx)
            cls.append(c)
            loc.append(l)

        if self.weighted:
            cls_weight = F.softmax(self.cls_weight, 0) # dim=0，对每一列进行softmax
